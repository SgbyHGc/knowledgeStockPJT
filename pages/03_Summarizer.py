...
def gemini(extacted_text, api_key):
  ...
  ■ 2. MMMによる包括的な効果測定: 独自のMMMを開発し、オンライン・オフラインを含む多様なプロモーション施策のインストール数への貢献度を分析。施策間相互作用も考慮した予算配分や、予測値と実測値の誤差が少ない高精度なモデル構築を実現。
  ■ 3. CausalImpactによる認知施策の効果検証: 従来、懐疑的な見方があった認知施策の効果を、CausalImpactを用いた分析により定量化。YouTube広告配信によるインストール数の純増や、ランディングページへの遷移率向上を実証し、認知施策の貢献度を明確化。
  ■ 4. iOSアプリ広告における「推定ROAS」の活用: ATT導入によるiOSアプリ広告の効果測定の課題に対し、「推定ROAS」を開発・運用。従来のROASに近い数値を算出することで、iOSアプリキャンペーンの出稿タイトル数増加とROAS目標達成を実現。
  ■ 5. 多角的な検証による組織風土の醸成: MMM、CausalImpact、「推定ROAS」といった多角的な検証体制の構築により、社内のデータに基づいた意思決定を促進。データドリブンなマーケティングへの意識改革を推進し、組織全体の「測定」に対する意識向上を実現。
